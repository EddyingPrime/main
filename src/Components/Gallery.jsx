...
const Carousel = ({ items }) => {
    ...
    const prevSlide = () => {
        setCurrentSlide((currentSlide - 1 + (items.length - slidesToShow + 1)) % (items.length - slidesToShow + 1));
    };
    ...
    return (
        <div className="carousel" style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div className="carousel-content" style={{ display: 'flex', flex: '1', justifyContent: 'space-between' }}>
                {visibleItems.map((item, index) => (
                    <img key={index} src={item} alt={`Item ${index}`} style={{ width: '300px', height: '200px', marginRight: '10px' }} />
                ))}
            </div>
            <div style={{ order: 3 }}>
...
export default function Gallery() {
    ...
    return (
        <>
            <div className="w-full bg-parallax bg-cover bg-fixed bg-no-repeat h-40 mt-8
